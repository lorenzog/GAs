 ...
 */
void set_version_parameters(int version) {
...
inline void print_usage(char *prog_name) {
	printf("Usage: %s --version VERSION [--OPTIONS (value)]\n", prog_name);
	printf("version is 1 (ECAL) or 2 (JTB)\n");
...
/**
 * prints the parameters used for the current simulation
 */
void print_parameters() {
	fprintf(stderr, "num_runs: %d\nmut_rate: %f\ntimesteps: %d\n", 
			NUM_RUNS, MUT_RATE, TIMESTEPS);
	fprintf(stderr, "pert_min: %f\npert_max: %f\na_start: %f\na_end: %f\n",
			PERT_MIN, PERT_MAX, A_START, A_END);
	fprintf(stderr, "intervals: %d\npop_size: %d\ndeath_rate: %f\n", 
			INTERVALS, POP_SIZE, DEATH_RATE);
	fprintf(stderr, "lambda: %f\nalpha: %f\nbeta: %f\nres_start: %f\nres_end: %f\n", 
			LAMBDA, ALPHA, BETA, RESOURCE_START, RESOURCE_END);
	fprintf(stderr, "using version: %d\n", VERSION);
}
...
bool parse_options(int argc, char** argv) {
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "-n:s:m:d:l:a:b:t:r:R:p:P:j:k:M:x:i:v:",
		...
		switch (c) {
			...
			case 'v':
				VERSION = atoi(optarg);
				set_version_parameters(VERSION);
				break;
			default:
